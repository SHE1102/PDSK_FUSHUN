						...
						<div class="tab-content no-border padding-24" id="tab">
							<div id="faq-tab-1" class="tab-pane fade in active">
								<div class="page-content">
									<div class="row">
										<div class="col-xs-12">  
										 	<form action="<%=request.getContextPath()%>/stationQuery" method="get">
												<div class="row">
													<div class="col-xs-12">
														  <div class="table-header">
															<small>数据查询</small> 
														<!-- <a href="#" onclick=preview(1) class="pull-right" style="margin-right: 15px;"><button type="button"
															class="btn btn-primary">
															<i class="icon-print"></i><small>打印</small>
													         </button></a>  -->
												...
												<div class="table-responsive">
															...
															<tr>
																...
																<td style="line-height: 20px;">
																 	<span style="position: relative;top: 3px;">
																		<select name="model" style="width:100px;" id="sel">
																		   <c:if test="${queryInfo.model !=null}">
																				<option selected="selected" value="${queryInfo.model }">
																					...
																					<c:if test="${queryInfo.model ==2}">单期变形量</c:if>
																					<c:if test="${queryInfo.model ==3}">累计变形量</c:if>
																				</option>
																			</c:if>
																				<!-- <option value="1">全部查询</option> -->
																				<option value="2">单期变形量</option>
																		...
																		阈值:
																		<input id="num" name="threshold" value="${queryInfo.threshold}" type="text" onkeyup="value=value.replace(/[^\-?\d.]/g,'')" pattern="^[0-9.]*$" required oninvalid="setCustomValidity('请输入一个大于零的数');" oninput="setCustomValidity('');" style="width: 80px">(mm)
																		<br>
																...
																<!-- style="position: absolute;margin:20px 1px;"  -->
																<!-- 查询时间 -->
																<td class="center" style="line-height: 20px;position: relative;top:10px;"> 
																	...
																	&nbsp;<span class="sp" >-</span>&nbsp; 
																	<input id="startTime" class="Wdate" name="time3" value="${queryInfo.timeStart }" style="height: 30px;width:130px;" readonly="true" onclick="createWdatePicker();" onfocus="createWdatePicker();" />
																	<i style="position: relative;top:0px;left:-25px;" class="icon-chevron-down sp" ></i>
																</td>

																<!-- 选中时间段 -->
																<td  style="line-height: 20px;position: relative;top:10px;">
																	<input id="showstarttime" name="timeStart" type="text" style="height: 30px;width:150px;"  value="${queryInfo.timeStart }"/>
																	 &nbsp;<span class="sp">-</span>&nbsp;
																	<input id="showendtime" name="timeEnd" type="text" style="height: 30px;width:150px;"  value="${queryInfo.timeEnd }"/>
																</td>

															     <!-- 抚顺版 --> 
															     <%-- <td  style="line-height: 20px;position: relative;top:15px;width:60px;">
															         
																	 <c:forEach items="${MSments }" var="MS">
																	 	 ${MS.name } 
																	 </c:forEach>
																 </td>  --%>
																...
																<td style="line-height: 20px;width:450px;">
																	<c:forEach var="monitorStation" items="${monitorStationList}">
																		<input type="submit" class="btn btn-primary" name="monitorStationName" value="${monitorStation.name }" style="position: relative;top:6px;margin: 5px;"/>
																	</c:forEach>
																</td>
															</tr> 
														</thead>
													...
													<table id="sample-table-2" class="table table-striped table-bordered table-hover">
														<thead> 
															<tr>
																<!--
																<th class="center" rowspan="4">
																	<label>
																		<input type="checkbox" class="ace" id="all" onclick="getAll()"/>
																		<span class="lbl"></span>
																	</label>
																</th> -->
																<th rowspan="3">编号</th> 
														...
														<!--查询记录-->
														<tbody> 
															<c:forEach var="station" items="${page.stationList}" varStatus="status">
																	<tr id="hh">
																		<!--
																		<td class="center">
																			<label>
																				<input type="checkbox" name="check" class="ace" id="ace"/>
																				<span class="lbl"></span>
																			</label>
																		</td> -->
															...
															<!--分页-->
															<tr align="center">
																<td colspan="15" style="position: relative;top:-5px;">
																	<ul class="pagination">
																		<li>
																			<a href="<%=request.getContextPath()%>/stationQuery/1/${queryInfo.model}/${queryInfo.threshold}/${queryInfo.x}/${queryInfo.y}/${queryInfo.h}/${queryInfo.dateModel }/${queryInfo.timeStart }/${queryInfo.timeEnd }/${queryInfo.baseStationName}/${queryInfo.monitorStationName}">首页</a>
																		</li>
																		<li>
																			<c:if test="${page.currPageNum>1}">
																				<a href="<%=request.getContextPath()%>/stationQuery/${page.currPageNum-1}/${queryInfo.model}/${queryInfo.threshold}/${queryInfo.x}/${queryInfo.y}/${queryInfo.h}/${queryInfo.dateModel }/${queryInfo.timeStart }/${queryInfo.timeEnd}/${queryInfo.baseStationName}/${queryInfo.monitorStationName}">上一页</a>
																			</c:if>
																		</li>
																		<li>
																		...
																		<li>
																			<c:if test="${page.currPageNum < page.totalPageSize}">
																				<a href="<%=request.getContextPath()%>/stationQuery/${page.currPageNum+1}/${queryInfo.model}/${queryInfo.threshold}/${queryInfo.x}/${queryInfo.y}/${queryInfo.h}/${queryInfo.dateModel}/${queryInfo.timeStart}/${queryInfo.timeEnd}/${queryInfo.baseStationName}/${queryInfo.monitorStationName}">下一页</a>
																			</c:if>
																		</li>
																		<li>
																			<a href="<%=request.getContextPath()%>/stationQuery/${page.totalPageSize}/${queryInfo.model}/${queryInfo.threshold}/${queryInfo.x}/${queryInfo.y}/${queryInfo.h}/${queryInfo.dateModel }/${queryInfo.timeStart }/${queryInfo.timeEnd}/${queryInfo.baseStationName}/${queryInfo.monitorStationName}">尾页</a>
																		</li>
																	</ul>
																	&nbsp;<span>当前第<span style="color: #307ECC;font-weight: bold;">
																	<c:if test="${page.totalPageSize != 0}">${page.currPageNum}</c:if>
													   ...
													   <script type="text/javascript">
														...
														<script type="text/javascript">
															...
															function chose(){  
																...
																if(num == '${page.currPageNum}'-1 || num > '${page.totalPageSize}'){
																	/* alert('输入的页码有误,请重新输入!');
																	return false; */
																	$('#light').show();
	...
	<script> 
		...
		$exportLink.addEventListener('click', function(e){ 
			...
			if(e.target.nodeName === "A"){
				...
				tableExport('sample-table-2', '数据查询', e.target.getAttribute('data-type'));
